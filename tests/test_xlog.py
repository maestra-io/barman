...
class Test(object):
    def test_encode_segment_name(self):
        assert xlog.encode_segment_name(0, 0, 0) == "000000000000000000000000"
        assert xlog.encode_segment_name(1, 1, 1) == "000000010000000100000001"
        assert xlog.encode_segment_name(10, 10, 10) == "0000000A0000000A0000000A"
        assert xlog.encode_segment_name(17, 17, 17) == "000000110000001100000011"
        assert xlog.encode_segment_name(0, 2, 1) == "000000000000000200000001"
        assert xlog.encode_segment_name(1, 0, 2) == "000000010000000000000002"
        assert xlog.encode_segment_name(2, 1, 0) == "000000020000000100000000"

    def test_decode_segment_name(self):
        assert xlog.decode_segment_name("000000000000000000000000") == [0, 0, 0]
        assert xlog.decode_segment_name("000000010000000100000001") == [1, 1, 1]
        assert xlog.decode_segment_name("0000000A0000000A0000000A") == [10, 10, 10]
        assert xlog.decode_segment_name("000000110000001100000011") == [17, 17, 17]
        assert xlog.decode_segment_name("000000000000000200000001") == [0, 2, 1]
        assert xlog.decode_segment_name("000000010000000000000002") == [1, 0, 2]
        assert xlog.decode_segment_name("000000020000000100000000") == [2, 1, 0]
        assert xlog.decode_segment_name("00000001000000000000000A.00000020.backup") == [
            1,
            0,
            10,
        ]
        assert xlog.decode_segment_name("00000001.history") == [1, None, None]
    ...
    def test_generate_segment_names_xlog_file_size_known(self):
        ...
        assert (
            sum(
                1
                for _ in xlog.generate_segment_names(
    ...
    def test_generate_segment_names_xlog_file_size_unknown(self):
        assert tuple(
            xlog.generate_segment_names(
                "0000000100000001000000FD", "000000010000000100000102", 90200
            )
        ) == (
            "0000000100000001000000FD",
            "0000000100000001000000FE",
        ...
        assert tuple(
            xlog.generate_segment_names(
                "00000001000000010007FFFE", "000000010000000200000002", 90300
            )
        ) == (
        ...
        assert tuple(
            xlog.generate_segment_names(
                "00000001000000010007FFFE", "000000010000000200000002", 90200
            )
        ) == (
            "00000001000000010007FFFE",
            "000000010000000200000000",
    ...
    def test_is_any_xlog_file(self):
        ...
        assert xlog.is_any_xlog_file("00000001000000000000000A.00000020.backup")
        assert xlog.is_any_xlog_file("test2/00000001000000000000000A.00000020.backup")
        assert xlog.is_any_xlog_file("00000001000000000000000A.partial")
        assert xlog.is_any_xlog_file("test2/00000001000000000000000A.partial")
    ...
    def test_backup_file(self):
        ...
        assert not xlog.is_backup_file("00000001000000000000000A.00000020.partial")
    ...
    def test_parse_lsn(self):
        ...
        assert xlog.parse_lsn("FFFFFFFF/FFFFFFFF") == ((0xFFFFFFFF << 32) + 0xFFFFFFFF)
    ...
    def test_location_to_xlogfile_name_offset(self, size, name, offset, lsn):
        result = xlog.location_to_xlogfile_name_offset(lsn, 3, 1 << size)
        assert result == {
            "file_name": name,
            "file_offset": offset,
        }
...
class TestCheckArchiveUsable(object):
    ...
    EXPECTED_ARCHIVE_CONTENT_ERROR = (
        "Found %s file%s in WAL archive equal to or newer than timeline %s"
    )
    ...
    def test_error_if_timeline_is_malformed(self, bad_timeline):
        ...
        assert (
            "Cannot check WAL archive with malformed timeline %s" % bad_timeline
            == str(exc.value)
        )

    def test_passes_if_timeline_with_no_wals(self):
        xlog.check_archive_usable([], timeline=1)
    ...
    def test_passes_if_timeline_gt_history_file(self, wals, timeline):
        xlog.check_archive_usable(
            wals,
            timeline=timeline,
        )
    ...
    @pytest.mark.parametrize(
        "wals,timeline,num_expected_newer_files",
        [
            (["00000002.history"], 1, 1),
            (["00000002.history", "00000003.history"], 2, 2),
            (["00000002.history", "00000003.history"], 3, 1),
            (["00000003.history", "00000002.history"], 2, 2),
            (["00000003.history", "00000002.history"], 3, 1),
        ],
    )
    def test_fails_if_timeline_lte_history_file(
        self, wals, timeline, num_expected_newer_files
    ):
        with pytest.raises(WalArchiveContentError) as exc:
            xlog.check_archive_usable(
                wals,
    ...
    def test_passes_if_timeline_gt_wal_file_timeline(self, wals, timeline):
        xlog.check_archive_usable(
            wals,
            timeline=timeline,
        )
    ...
    @pytest.mark.parametrize(
        "wals,timeline,num_expected_illegal_files",
        [
    ...
    def test_fails_if_timeline_lte_wal_file_timeline(
        self, wals, timeline, num_expected_illegal_files
    ):
        with pytest.raises(WalArchiveContentError) as exc:
            xlog.check_archive_usable(
                wals,
